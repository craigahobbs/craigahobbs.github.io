...
function main()
    ...
    jumpif (vPrint) skipMenu
        markdownPrint( \
            '[Reset](#var=) |', \
            '[Small](#var.vWidth=400&var.vHeight=250' + if(vMessage, "&var.vMessage='" + markdownEscapeURLComponent(vMessage) + "'", '') + ') |', \
            '[Medium](#var.vWidth=700&var.vHeight=350' + if(vMessage, "&var.vMessage='" + markdownEscapeURLComponent(vMessage) + "'", '') + ') |', \
            '[Large](#var.vWidth=1000&var.vHeight=450' + if(vMessage, "&var.vMessage='" + markdownEscapeURLComponent(vMessage) + "'", '') + ') |', \
            ...
            '[Print](#var.vPrint=1' + if(vWidth, '&var.vWidth=' + vWidth, '') + if(vHeight, '&var.vHeight=' + vHeight, '') + \
                if(vMessage, "&var.vMessage='" + markdownEscapeURLComponent(vMessage) + "'", '') + ')' \
        )
    skipMenu:
